...
/******************************************************************************/
#include "MsgStore.h"      /* Msg Store Implementation Prototypes/Constants.   */

#include "BTPSKRNL.h"      /* Bluetooth O/S Abstraction Prototypes/Constants.  */
...
char *GetCurrentMessageFolderString(void)
{
   ...
   /* The following function is a utility function that is provided to  */
   /* allow a mechanism to change folders (and thus update the Current  */
...
int QueryNumberFolderEntries(CurrentFolder_t Folder)
{
   ...
   /* OK, we need to locate any directories that have the specified     */
   ...
   /* The following function is a utility function that is provided to  */
   /* allow a mechanism to query information about a specific folder    */
   ...
   for(Index1=0,Index2=0,ret_val=FALSE;(Index1<sizeof(FolderStructure)/sizeof(FolderStructure_t));Index1++)
   {
      if(FolderStructure[Index1].ParentFolder == Folder)
      {
         ...
         if(Index == Index2)
         {
            ...
            /* Break out of outer loop as match has been found.         */
            break;
         }
...
int QueryNumberMessageEntries(CurrentFolder_t Folder)
{
   ...
   /* Simply loop through the Folder Message Mapping array until we find*/
   ...
   /* The following function is a utility function that is provided to  */
   /* allow a mechanism to query information about a specific message   */
...
Boolean_t QueryMessageEntryByIndex(CurrentFolder_t Folder, unsigned int Index, MessageEntry_t **MessageEntry)
{
   ...
   /* Simply loop through the Folder Message Mapping array entail we    */
...
Boolean_t QueryMessageEntryByHandle(CurrentFolder_t Folder, char *MessageHandle, MessageEntry_t **MessageEntry)
{
   ...
   /* Simply loop through the Folder Message Mapping array in til we    */
   /* find the specified folder.                                        */
   ...
   /* The following function is a utility function that is provided to  */
   /* allow a mechanism to move a message from one folder to another.   */
...
void InsertMessageEntryIntoFolder(CurrentFolder_t Folder, char *MessageHandle)
{
   ...
   /* The following function is a utility function that is provided to  */
   /* allow a mechanism to retrieve the Event Type (in string form).    */
char *GetEventTypeString(EventType_t EventType)
{
   /* Verify that the value passed in is in the valid range.            */
   return((char *)(((unsigned int)EventType <= etMessageDeleted) ? EventTypeStr[EventType] : NULL));
}

   /* The following function is a utility function that is provided to  */
   /* allow a mechanism to retrieve the Message Type (in string form).  */
char *GetMessageTypeString(MessageType_t MessageType)
{
   /* Verify that the value passed in is in the valid range.            */
   return((char *)(((unsigned int)MessageType <= mtMMS) ? MessageTypeStr[MessageType] : NULL));
}

   /* The following function is a utility function that is provided to  */
   /* allow a mechanism to generate a unique Message Handle that can be */
