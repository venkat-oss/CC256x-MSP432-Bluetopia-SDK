   ...
   if(CommandParser(&TempCommand, Command) >= 0)
   {
      ...
      switch(Result)
      {
         ...
         case FUNCTION_ERROR:
            ...
            break;
         case EXIT_CODE:
            /* Check to see if there is currently a Connection or Server*/
            /* Open.                                                    */
            if(PBAPID)
            {
               /* There is currently a Server or Connection open, check */
               /* to see if the Current Service Type is Client.         */
               if(UI_Mode == UI_MODE_IS_CLIENT)
               {
                  /* There is a Client Connection open, Close the       */
                  /* Connection.                                        */
   ...
   if(BluetoothStackID)
   {
      ...
      if(PBAPID)
      {
         ...
         if(!Result)
         {
            ...
            /* PBAP_Close_Connection() the connection is dropped.       */
            if (UI_Mode == UI_MODE_IS_CLIENT)
            {
                PBAPID = 0;
            }
